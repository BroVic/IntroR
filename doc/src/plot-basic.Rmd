...
## Graph/Chart Annotation
A graph or chart is meaningless without title, labels, legends and other kinds of annotation.
...
```{r main, eval=FALSE}
hist(df$Wind,
     main = "Wind Speed for New York, May - Sept 1973")
```
...
hist(df$Wind,
     ...
     xlab = "Wind Speed (mph)",
     ylab = "No. of observations",
     labels = LETTERS[1:11])        # earlier counted 11 bars
```
...
***
Recall the `table()` function, which gives us the count of each discrete countable variable.
```{r table-heat}
table(df$Heat)
```
...
This is the default call on `barplot`. Use `?` to take a look at the arguments.
...
- We have only scratched the surface with these examples.
